...
@section('content')
    <main class="flex-1 p-4 md:p-6">
        ...
        <div class="page-header">
            ...
            <p class="page-subtitle">{{ __('Update the article content for your platform visitors') }}</p>
        </div>
            ...
            <form action="{{ route('admin.posts.update', $post->id) }}" method="post" id="postForm">
                ...
                <div class="form-section">
                    ...
                    <input type="text" name="title" id="postTitle" value="{{ old('title', $post->title) }}"
                        ...
                        placeholder="{{ __('Write an attractive title for the post...') }}" required
                ...
                <div class="form-section">
                    ...
                    <select name="auther_id" class="form-select @error('author_id') error @enderror" required>
                        <option value="">{{ __('Select an author') }}</option>
                        @foreach($authors as $author)
                            <option value="{{ $author->id }}" {{ old('author_id', $post->auther_id) == $author->id ? 'selected' : '' }}>
                                {{ $author->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-section">
                    ...
                    <div class="editor-wrapper">
                        ...
                        <div class="editor-toolbar">
                            ...
                            <div class="toolbar-group">
                                <button type="button" class="toolbar-btn" onclick="insertMarkdown('**', '**')"
                                    data-tooltip="{{ __('Bold') }}">
                                    <i class="fas fa-bold"></i>
                                </button>
                                <button type="button" class="toolbar-btn" onclick="insertMarkdown('*', '*')"
                                    data-tooltip="{{ __('Italic') }}">
                            ...
                            <div class="toolbar-group">
                                <button type="button" class="toolbar-btn" onclick="insertMarkdown('## ', '')"
                                    data-tooltip="{{ __('Heading 2') }}">
                                    <i class="fas fa-heading"></i><span class="text-xs">2</span>
                                </button>
                                <button type="button" class="toolbar-btn" onclick="insertMarkdown('### ', '')"
                                    data-tooltip="{{ __('Heading 3') }}">
                            ...
                            <div class="toolbar-group">
                                <button type="button" class="toolbar-btn" onclick="insertMarkdown('`', '`')"
                                    data-tooltip="{{ __('Inline Code') }}">
                                    <i class="fas fa-code"></i>
                                </button>
                                <button type="button" class="toolbar-btn" onclick="insertMarkdown('> ', '')"
                                    data-tooltip="{{ __('Blockquote') }}">
                            ...
                            <!-- Table & HR -->
                            <div class="toolbar-group">
                                <button type="button" class="toolbar-btn" onclick="insertTable()"
                                    data-tooltip="{{ __('Insert Table') }}">
                                    <i class="fas fa-table"></i>
                                </button>
                                <button type="button" class="toolbar-btn" onclick="insertMarkdown('\n---\n', '')"
                                    data-tooltip="{{ __('Horizontal Line') }}">
                            ...
                            <div class="toolbar-group" style="margin-right: auto;">
                                <button type="button" class="toolbar-btn" onclick="openHelp()"
                                    data-tooltip="{{ __('Formatting Guide') }}">
                        ...
                        <!-- Editor/Preview Area -->
                        <textarea id="postBody" name="body" class="editor-textarea @error('body') error @enderror"
                            placeholder="{{ __('Write your content here using markdown syntax...') }}" required
                            oninput="updatePreview(); updateStats();">{{ old('body', $post->body) }}</textarea>
                        ...
                        <div class="editor-stats">
                            <div class="stats-left">
                                <span class="stat-item">
                                    ...
                                    <span class="stat-value" id="wordCount">0</span> {{ __('words') }}
                                </span>
                                <span class="stat-item">
                                    <i class="fas fa-clock"></i>
                                    <span class="stat-value" id="charCount">0</span> {{ __('characters') }}
                                </span>
                                <span class="stat-item">
                                    <i class="fas fa-stopwatch"></i>
                                    <span class="stat-value" id="readTime">1</span> {{ __('min read') }}
                                </span>
                            </div>
                ...
                <div class="form-actions">
                    ...
                    <button type="submit" class="btn btn-primary" id="submitBtn">
                        <i class="fas fa-paper-plane"></i> {{ __('Update Post') }}
                    </button>
                </div>
            </form>
        </div>
    ...
    <script>
        (function () {
            ...
            window.insertMarkdown = function (prefix, suffix) {
                ...
                // Trigger input event for stats/preview
            ...
            window.insertImage = function () {
                ...
                // Simple prompt for URL (can be enhanced with modal later)
                const url = prompt('أدخل رابط الصورة:\nمثال: https://example.com/image.jpg', 'https://');
                if (!url || url === 'https://') return;

                const caption = prompt('أدخل وصف الصورة (اختياري):', '');
            ...
            window.insertLink = function () {
                ...
                const text = prompt('نص الرابط:', 'اضغط هنا');
                if (!text) return;

                const url = prompt('أدخل الرابط:\nمثال: https://example.com', 'https://');
                if (!url || url === 'https://') return;
            ...
            function parseMarkdown(text) {
                if (!text) return '<p class="text-muted">اكتب شيئاً للمعاينة...</p>';
                ...
                html = html.replace(/\[img:([^\]|]+)(?:\|([^\]]+))?\]/g, (match, url, caption) => {
                    ...
                    return `<figure style="text-align:center;margin:1.5rem auto;max-width:100%">
                            <img src="${cleanUrl}" alt="${cleanCaption}" style="max-width:100%;height:auto;border-radius:12px;box-shadow:0 4px 6px -1px rgba(0,0,0,0.1);border:3px solid white" onerror="this.parentElement.innerHTML='<p style=\'color:#ef4444\'>❌ فشل تحميل الصورة</p>'">
                    ...
                    if (trimmed.startsWith('|') && trimmed.endsWith('|')) {
                        ...
                        if (!trimmed.match(/^\|[ -:|]+\|$/)) {
                            const cells = trimmed.split('|')
                                .slice(1, -1)
                                .map(cell => `<td style="padding:0.75rem 1rem;border-bottom:1px solid #e2e8f0">${cell.trim()}</td>`)
                                .join('');
                            tableRows.push(`<tr>${cells}</tr>`);
                        }
                    } else {
                        if (inTable && tableRows.length > 0) {
                            const header = tableRows.shift().replace(/<td>/g, '<th style="padding:0.875rem 1rem;text-align:right;font-weight:600;color:#1e293b;border-bottom:2px solid #bfdbfe;background:#eff6ff">').replace(/<\/td>/g, '</th>');
                            result.push(`<table style="width:100%;border-collapse:collapse;margin:1.25rem 0;background:white;border-radius:10px;overflow:hidden"><thead>${header}</thead><tbody>${tableRows.join('')}</tbody></table>`);
                            inTable = false;
                            tableRows = [];
                        }
                        result.push(line);
            ...
            function parseOrderedLists(html) {
                ...
                for (let line of lines) {
                    ...
                    if (trimmed.match(/^\d+\.\s+.+$/)) {
                        ...
                        result.push(`<li style="margin:0.5rem 0;padding:0.25rem 0 0.25rem 2rem;position:relative;counter-increment:item;line-height:1.7"><span style="position:absolute;right:0;color:#2563eb;font-weight:700;min-width:1.5rem" data-count="${counter++}">${counter - 1}.</span>${content}</li>`);
                    } else {
            ...
            function parseParagraphs(html) {
                const blocks = html.split(/(<\/?(?:h[2-6]|ul|ol|li|table|thead|tbody|tr|th|td|blockquote|pre|hr|figure|figcaption|code)[^>]*>)/i);
            ...
            function setupAutoSave() {
                ...
                const originalText = statusText?.textContent || 'Auto-saved';

                ta.addEventListener('input', () => {
                    if (status.classList.contains('saving')) return;

                    status.classList.add('saving');
                    if (statusText) statusText.textContent = 'جاري الحفظ...';
            ...
            function setupKeyboardShortcuts() {
                document.addEventListener('keydown', (e) => {
                    ...
                    // Only work when textarea is focused
            ...
            // This ensures onclick="functionName()" works from HTML
