...
@section('content')
    <main class="flex-1 p-6 md:ml-0">
            <div class="mb-8">
                <h1 class="text-3xl font-bold text-gray-800">{{ __('Edit Article') }}</h1>
                <p class="text-gray-600 mt-2">{{ __('Update the article content for your platform visitors') }}</p>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-xl overflow-hidden max-w-4xl mx-auto">
                ...
                <div class="p-6">
                    <form action="{{ route('admin.posts.update', $post->id) }}" method="post" class="space-y-8">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <select 
                                name="auther_id" 
                                class="form-input w-full px-9 py-3 border @error('author_id') border-red-500 @else border-gray-300 @enderror rounded-xl" 
                                required
                            >
                        ...
                        <div class="flex gap-4 pt-4">
                            ...
                            <button type="submit" class="px-6 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-xl hover:from-indigo-700 hover:to-purple-700 transition shadow-lg">
                                <i class="fas fa-paper-plane mr-2"></i>{{ __('Update Post') }}
                            </button>
                        </div>
                    </form>
                </div>
